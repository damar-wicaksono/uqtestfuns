...
New methods and algorithms are continuously being developed within
...
All in all, `UQTestFuns` aims to save the researchers' and developers' time
from reimplementing many of the commonly used test functions and provide
...
developed.
While such methods are eventually aimed at solving
real-world problems&mdash;typically involve complex expensive-to-evaluate
computational models&mdash;researchers and developers may prefer to initially
use the so-called UQ test functions for validation and benchmarking purposes.

UQ test functions are, in principle, mathematical functions taken
as black boxes; they take a set of input values and produce output values.
...
- test functions are _fast to evaluate_, at least _faster_
...
- test functions provide _a common ground_ for comparing the performance of
  ...
  in solving the same class of problems.
...
- The Virtual Library of Simulation Experiments (VLSE) [@VLSE:2013]:
  This site is arguably the definitive repository
  for (but not exclusively) UQ test functions.
...
Below are some examples from the applied UQ community in Python
...
Furthermore, test functions from a given package
are often implemented in such a way
...
that requires combining a test function
...
equipped with a joint probability density function (PDF)
...
$$
where $\mathbf{x} = \{ \sigma_f, \theta \}$ is the two-dimensional vector of
...
$$
where $g(\mathbf{x}) \leq 0.0$ is defined as a _failure state_.
...
with a set of input values.
As explained, the probabilistic input model is an integral part
of a UQ test function.
In `UQTestFuns`, the input model according to the original specification
is attached to the instance of the test function:
```
...
to transform a set of values in a given domain
(say, the unit hypercube $[0, 1]^M$) to the domain of the test function.
...
the task of the analysis is to accurately estimate the probability
where $g(\mathbf{X}) \leq 0$
