...
(test-functions:bratley-a)=
# Product-of-Absolutes Function from Bratley et al. (1992)
...
The `BratleyA` function is an $M$-dimensional scalar-valued function
defined as the product of its inputs transformed absolute values.
...
```{note}
There are three other test functions
used in Bratley et al. {cite}`Bratley1992`:
...
The plots for one-dimensional and two-dimensional `BratleyA` functions
are shown below.
...
# --- Create 1D data
my_bratley_a_1d = uqtf.BratleyA(input_dimension=1)
xx_1d = np.linspace(0, 1, 1000)[:, np.newaxis]
yy_1d = my_bratley_a_1d(xx_1d)

# --- Create 2D data
my_bratley_a_2d = uqtf.BratleyA(input_dimension=2)
mesh_2d = np.meshgrid(xx_1d, xx_1d)
xx_2d = np.array(mesh_2d).T.reshape(-1, 2)
yy_2d = my_bratley_a_2d(xx_2d)
...
axs_2.set_title("Surface plot of 2D BratleyA", fontsize=14)
...
axs_3.set_title("Contour plot of 2D BratleyA", fontsize=14)
...
```{code-cell} ipython3
my_testfun = uqtf.BratleyA(input_dimension=6)
```
...
The `BratleyA` function is defined as follows[^location]:
