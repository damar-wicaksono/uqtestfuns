...
The (4th) Franke function (`Franke4` for short) is a two-dimensional
scalar-valued function introduced in {cite}`Franke1979` for scattered
data interpolation. It is a modified form of the
{ref}`McLain S3 function <test-functions:mclain-s3>` {cite}`McLain1974`,
featuring a gentle Gaussian hill centered at $(0.5, 0.5)$.
...
The function reaches its maximum of $\frac{1}{3}$ at $(0.5, 0.5)$.

```{note}
The (4th) Franke function is a modified form
of the {ref}`McLain S3 function <test-functions:mclain-s3>` {cite}`McLain1974`.
