...
# Continuous Function from Genz (1984)
...
The Genz continuous function (or `GenzContinuous` for short)
is an $M$-dimensional scalar-valued function
featuring exponential decay from the center of the input domain.
It is continuous everywhere but non-differentiable at the center,
making it a useful benchmark for assessing the accuracy
...
The parameters of the Genz continuous function consist
of the vector of shape (scale) parameters $\boldsymbol{a}$
and the vector of offset parameters $\boldsymbol{b}$. 

The shape parameter controls the rate of exponential decay;
larger values make the decay steeper and the integration more challenging.
